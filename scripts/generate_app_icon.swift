...
/// Builds a tinted SF Symbol image using the requested weight and size.
/// - Parameters:
///   - name: The SF Symbol name.
///   - pointSize: The point size to request from the symbol configuration.
///   - weight: The font weight to apply to the symbol.
///   - color: The tint color to composite over the symbol.
/// - Returns: A tinted symbol image, or `nil` when the symbol cannot be created.
func tintedSymbol(name: String, pointSize: CGFloat, weight: NSFont.Weight, color: NSColor)
  -> NSImage?
{
...
/// Renders the master 1024x1024 icon artwork.
/// - Returns: The fully composed app icon image.
func renderMasterImage() -> NSImage {
  ...
  let trackPath = NSBezierPath(
    roundedRect: trackRect, xRadius: trackRect.height / 2, yRadius: trackRect.height / 2)
...
/// - Returns: PNG data, or `nil` when encoding fails.
...
/// Renders an `NSImage` into a square `CGImage` at a specific pixel size.
/// - Parameters:
///   - image: The source image to rasterize.
///   - pixelSize: The target width and height in pixels.
/// - Returns: A rasterized image, or `nil` when rendering fails.
...
case "icns":
  ...
  guard let destination = CGImageDestinationCreateWithURL(outputURL as CFURL, iconType, 0, nil)
  else {
