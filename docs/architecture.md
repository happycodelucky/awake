...
| `Components.swift` | `TimerHeroView`, `CircleActionIcon`, `PolicyWarningCard`, `UpdateNoticeCard` | Self-contained SwiftUI view components. `TimerHeroView` renders the racetrack ring progress, large countdown text, and context-sensitive action button. `PolicyWarningCard` shows expandable MDM policy warnings. `UpdateNoticeCard` shows update state and action buttons. |
