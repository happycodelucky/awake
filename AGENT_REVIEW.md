...
| 2026-03-10 | Used `.onOpenURL` as the sole URL handler for MenuBarExtra on macOS 26 (Tahoe) | URLs were silently dropped; `onOpenURL` does not fire for MenuBarExtra-only apps on macOS 26 | Use `NSApplicationDelegateAdaptor` with `application(_:open:)` as the primary URL handler; keep `onOpenURL` as a forward-compat fallback. Confirmed on macOS 26.2 build 25C56. |
| 2026-03-10 | Attempted to wire AwakeController to AppDelegate via `onAppear` and `_controller.wrappedValue` | Controller was nil when first URL arrived; URL dropped | `@MainActor` `AwakeController` must be owned by the `@MainActor`-annotated `AwakeAppDelegate` as a stored `let` property, created eagerly before the SwiftUI body runs |
